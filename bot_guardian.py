...
def fetch_guardian_article_text(link):
    """
    RSS گاردین فقط یک خلاصه‌ی کوتاه می‌دهد. این تابع خود صفحه‌ی خبر
    را باز کرده و متن کامل انگلیسی مقاله را استخراج می‌کند.

    سایت گاردین معمولاً بدنه‌ی اصلی خبر را داخل
    <div data-gu-name="body"> قرار می‌دهد. اگر این ساختار پیدا نشد
    (به‌خاطر تغییر طراحی سایت)، به fallback های ساده‌تر سوییچ می‌کند.
    """
    try:
        headers = {
            "User-Agent": (
        ...
        print(f"خطا در دریافت متن کامل خبر گاردین ({link}): {e}")
    return None
...
متن خبر زیر (از گاردین ورزشی) را به فارسی روان و طبیعی برگردان.
...
def main():
    ...
    source_name = feed.feed.get("title", "The Guardian Football")
    ...
    try:
        full_text = fetch_guardian_article_text(link)
        if not full_text:
