...
DELAY_BETWEEN_POSTS = 2 * 60  # 2 دقیقه
SOURCE_TAG = "@Ft360_ir"
...
def fetch_posts():
    ...
    posts = []
    blocks = re.findall(
        r'data-post="' + SOURCE_CHANNEL + r'/(\d+)".*?(?=data-post="' + SOURCE_CHANNEL + r'/\d+"|$)',
        html,
        re.DOTALL,
    )
    # روش دقیق‌تر: پیدا کردن هر بلوک پیام به‌صورت جداگانه
    raw_messages = re.split(r'(?=<div class="tgme_widget_message[^"]*" data-post="' + SOURCE_CHANNEL + r'/\d+")', html)
    ...
    for block in raw_messages:
        ...
        photo_match = re.search(r"tgme_widget_message_photo_wrap[^\"]*\"\s+style=\"background-image:url\('([^']+)'\)", block)
...
def main():
    ...
    if last_post_id is None:
        # اولین اجرا: فقط جدیدترین را ذخیره کن، چیزی پست نکن
